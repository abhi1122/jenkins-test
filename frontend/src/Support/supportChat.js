...
export default function SupportChat({ socket }) {
    ...
    useEffect(() => {
        ...
        socket.on("receiveMessage", (data) => {
            ...
            setInterval(() => {
                ...
                const offlineCustomer = Object.keys(newLastMessageTime).filter(val => {
    ...
    const sendMessage = (messageText = chatText) => {
        ...
        setChat((oldChat) => {
            ...
            let newChat = [];
            console.log(message, "...message");
            newChat = { message: [...message, { text: messageText, class: "right" }] };
    ...
    const logout = () => {
        if (Object.keys(chatList).length === 0) {
            ...
            socket.emit("supportOffline", { id: JSON.parse(localStorage.getItem('loginUser')).id });
